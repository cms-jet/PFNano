...
# keepInputs can take DeepCSV, DeepJet, DDX and ParT (any combination, or use empty placeholder list if no inputs are required)
...
def PFnano_customizeMC_add_DeepJet_ParT_and_Truth(process):
    addPFCands(process, True)
    add_BTV(process, True, keepInputs=['DeepCSV','DeepJet','DDX','ParT'], storeAK4Truth="yes")
    #process.NANOAODSIMoutput.fakeNameForCrab = cms.untracked.bool(True)  # needed for crab publication
    return process
...
def PFnano_customizeMC_allPF_add_DeepJet_ParT_and_Truth(process):
    addPFCands(process, True, True)
    add_BTV(process, True, keepInputs=['DeepCSV','DeepJet','DDX','ParT'], storeAK4Truth="yes")
    #process.NANOAODSIMoutput.fakeNameForCrab = cms.untracked.bool(True)  # needed for crab publication
    return process
...
def PFnano_customizeData_add_DeepJet_ParT(process):
    addPFCands(process, False)
    add_BTV(process, False, keepInputs=['DeepCSV','DeepJet','DDX','ParT'])
    #process.NANOAODSIMoutput.fakeNameForCrab = cms.untracked.bool(True)  # needed for crab publication
    return process
...
def PFnano_customizeData_allPF_add_DeepJet_ParT(process):
    addPFCands(process, False, True)
    add_BTV(process, False, keepInputs=['DeepCSV','DeepJet','DDX','ParT'])
    #process.NANOAODSIMoutput.fakeNameForCrab = cms.untracked.bool(True)  # needed for crab publication
    return process
